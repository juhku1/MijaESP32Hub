...
"html+='</p></div>';"
"if(isAdded){"
"html+='<button class=\"add-btn\" disabled>✓ Added</button>';"
"}else{"
...
"console.log('Removing device from main view:',addr);"
...
"if(event){event.stopPropagation();}"
"console.log('Adding device to main view:',addr);"
"if(openDeviceSettings){openDeviceSettings.classList.remove('open');openDeviceSettings=null;}"
"try{"
	"const resp=await fetch('/api/toggle-visibility',{method:'POST',"
"headers:{'Content-Type':'application/x-www-form-urlencoded'},"
...
"if(d.saved){"
"html+=' <div class=\"settings-option danger\" onclick=\"hideDevice(\\''+d.addr+'\\',event)\" style=\"margin-top:10px;\">➖ Remove from main view</div>';"
"}else{"
"html+=' <div class=\"settings-option\" onclick=\"showDevice(\\''+d.addr+'\\',event)\" style=\"margin-top:10px;background:#10b981;\">💾 Save to main view</div>'; }"
"html+=' </div></div>'"
"+' </div>';"
...
"let title='📊 '+name+' - ';"
"if(hours===1)title+='Last hour';"
"else if(hours===24)title+='Last 24 hours';"
"else if(hours===168)title+='Last week';"
"else title+='Last month';"
...
"catch(e){console.error('Error loading AIO settings:',e);}}else if(tab==='cloudflare'){"
"try{const r=await fetch('/api/d1/config');"
"if(!r.ok){console.error('Failed to load D1 settings:',r.status);return;}"
"const d=await r.json();"
"if(d.ok){document.getElementById('d1WorkerUrl').value=d.workerUrl||'';"
"document.getElementById('d1Enabled').checked=d.enabled;}}"
"catch(e){console.error('Error loading D1 settings:',e);}}else if(tab==='scan'){"
...
"const workerUrl=document.getElementById('d1WorkerUrl').value;"
"const token=document.getElementById('d1Token').value;"
"const enabled=document.getElementById('d1Enabled').checked;"
"if(!workerUrl||!token){alert('Enter Worker URL and API Token!');return;}"
"try{const r=await fetch('/api/d1/config',{method:'POST',headers:{'Content-Type':'application/json'},"
"body:JSON.stringify({workerUrl:workerUrl,token:token,enabled:enabled})});"
"const d=await r.json();if(d.ok){alert('D1 settings saved!');}else{alert('Error: '+d.error);}}"
"catch(e){alert('Error while saving: '+e.message);}}"
"async function testD1Connection(){"
"try{const r=await fetch('/api/d1/test',{method:'POST'});"
"const d=await r.json();if(d.ok){alert('✅ Connection successful!\\n\\n'+d.message);}else{alert('❌ Connection failed:\\n\\n'+d.error);}}"
"catch(e){alert('Error: '+e.message);}}"
...
"const d=await r.json();if(d.ok){alert('✅ Created '+d.count+' feeds!\\n\\nYou can now test sending.');}else{alert('Error: '+(d.error||'Unknown'));}"
"}catch(e){alert('Error: '+e.message);}}"
...
"function openD1Help(){"
"document.getElementById('d1HelpModal').classList.add('show');}"
"function closeD1Help(){"
"document.getElementById('d1HelpModal').classList.remove('show');}"
...
"<div class='modal-header'><h2>⚙️ Settings</h2>"
...
"<button class='settings-tab-btn' data-tab='cloudflare' onclick='switchSettingsTab(\"cloudflare\")'>🟠 Cloudflare D1</button>"
...
"</div>"
"<div id='tab-cloudflare' class='settings-tab-content'>"
"<div style='background:#0f172a;border:1px solid #334155;border-radius:12px;padding:16px;margin-bottom:20px;font-size:14px;color:#cbd5e1;line-height:1.6;'>"
"<strong style='color:#f1f5f9;'>📦 Long-term Data Archive</strong><br>"
"Store unlimited sensor history in your own Cloudflare D1 database (free 10GB). This is optional and works alongside Adafruit IO."
"</div>"
"<div style='background:#1e293b;border:2px solid #10b981;border-radius:12px;padding:20px;margin-bottom:20px;'>"
"<h3 style='color:#10b981;margin:0 0 15px 0;font-size:16px;'>🚀 Quick Setup Guide</h3>"
"<p style='margin:0 0 15px 0;color:#cbd5e1;font-size:14px;line-height:1.6;'>"
"Follow these steps to deploy your own Cloudflare Worker and D1 database:"
"</p>"
"<div style='background:#0f172a;border-left:3px solid #10b981;padding:15px;border-radius:6px;margin-bottom:15px;'>"
"<p style='margin:0 0 10px 0;color:#10b981;font-weight:600;font-size:14px;'>Step 1: Install Wrangler CLI</p>"
"<p style='margin:0 0 8px 0;color:#cbd5e1;font-size:13px;'>Open terminal and run:</p>"
"<code style='display:block;background:#1e293b;padding:10px;border-radius:6px;color:#10b981;font-family:monospace;font-size:13px;'>npm install -g wrangler</code>"
"<p style='margin:10px 0 0 0;color:#94a3b8;font-size:12px;'>💡 Need Node.js? Download from <a href='https://nodejs.org' target='_blank' style='color:#60a5fa;'>nodejs.org</a></p>"
"</div>"
"<div style='background:#0f172a;border-left:3px solid #10b981;padding:15px;border-radius:6px;margin-bottom:15px;'>"
"<p style='margin:0 0 10px 0;color:#10b981;font-weight:600;font-size:14px;'>Step 2: Navigate to cloudflare folder</p>"
"<p style='margin:0 0 8px 0;color:#cbd5e1;font-size:13px;'>Go to your project's cloudflare directory:</p>"
"<code style='display:block;background:#1e293b;padding:10px;border-radius:6px;color:#10b981;font-family:monospace;font-size:13px;'>cd MijiaESP32Hub/cloudflare</code>"
"<p style='margin:10px 0 0 0;color:#94a3b8;font-size:12px;'>💡 Adjust path based on where you downloaded the project</p>"
"</div>"
"<div style='background:#0f172a;border-left:3px solid #10b981;padding:15px;border-radius:6px;margin-bottom:15px;'>"
"<p style='margin:0 0 10px 0;color:#10b981;font-weight:600;font-size:14px;'>Step 3: Run setup script</p>"
"<p style='margin:0 0 8px 0;color:#cbd5e1;font-size:13px;'>Execute the automated setup:</p>"
"<code style='display:block;background:#1e293b;padding:10px;border-radius:6px;color:#10b981;font-family:monospace;font-size:13px;'>./setup.sh</code>"
"<p style='margin:10px 0 0 0;color:#cbd5e1;font-size:12px;line-height:1.6;'>"
"The script will:<br>"
"• Check Wrangler installation<br>"
"• Log you into Cloudflare (opens browser)<br>"
"• Create D1 database<br>"
"• Generate secure API token<br>"
"• Deploy Worker to Cloudflare"
"</p>"
"</div>"
"<div style='background:#0f172a;border-left:3px solid #fbbf24;padding:15px;border-radius:6px;margin-bottom:15px;'>"
"<p style='margin:0 0 10px 0;color:#fbbf24;font-weight:600;font-size:14px;'>Step 4: Copy credentials</p>"
"<p style='margin:0 0 8px 0;color:#cbd5e1;font-size:13px;'>At the end, the script shows:</p>"
"<div style='background:#1e293b;padding:10px;border-radius:6px;margin:8px 0;'>"
"<p style='margin:0;color:#94a3b8;font-size:12px;font-family:monospace;'>Worker URL: https://mijaesp32hub-d1-worker.xxx.workers.dev</p>"
"<p style='margin:5px 0 0 0;color:#94a3b8;font-size:12px;font-family:monospace;'>API Token: 1234abcd5678efgh...</p>"
"</div>"
"<p style='margin:0;color:#cbd5e1;font-size:13px;'>Copy these values to the fields below ⬇️</p>"
"</div>"
"</div>"
"<div style='background:#1e293b;border:1px solid #334155;border-radius:12px;padding:20px;margin-bottom:20px;'>"
"<h3 style='color:#cbd5e1;margin:0 0 15px 0;font-size:16px;'>⚙️ Configuration</h3>"
"<div style='margin-bottom:15px;'><label style='display:block;margin-bottom:5px;font-weight:600;'>Worker URL:</label>"
"<input type='text' id='d1WorkerUrl' placeholder='https://mijaesp32hub-d1-worker.xxx.workers.dev' style='width:100%;padding:10px;border:1px solid #334155;border-radius:8px;background:#0f172a;color:#e2e8f0;'></div>"
"<div style='margin-bottom:15px;'><label style='display:block;margin-bottom:5px;font-weight:600;'>Worker API Token:</label>"
"<input type='password' id='d1Token' placeholder='64-character hex token from setup script' style='width:100%;padding:10px;border:1px solid #334155;border-radius:8px;background:#0f172a;color:#e2e8f0;'></div>"
"<div style='margin-bottom:15px;'><label style='display:flex;align-items:center;gap:10px;'>"
"<input type='checkbox' id='d1Enabled' style='width:20px;height:20px;'><span>Archive enabled (stores all data)</span></label></div>"
"<button onclick='saveD1Settings()' style='width:100%;padding:12px;background:#10b981;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;margin-bottom:10px;'>💾 Save Settings</button>"
"<button onclick='testD1Connection()' style='width:100%;padding:12px;background:#3b82f6;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;margin-bottom:10px;'>🧪 Test Connection</button>"
"<button onclick='openD1Help()' style='width:100%;padding:10px;background:transparent;color:#94a3b8;border:1px solid #334155;border-radius:8px;font-weight:500;cursor:pointer;'>📖 Detailed Instructions & FAQ</button>"
"</div>"
"</div>"
...
"<li><strong>30-day data retention</strong> (use Cloudflare D1 for unlimited history)</li>"
"</ul>"
"</div>"
...
"</div></div></div>"
"<div id='d1HelpModal' class='modal'><div class='modal-content' style='max-width:700px;'>"
"<div class='modal-header'><h2>🚀 Cloudflare D1 Setup Guide</h2>"
"<button class='modal-close' onclick='closeD1Help()'>×</button></div>"
"<div style='padding:24px;'>"
"<div style='background:#0f172a;border-left:4px solid #f59e0b;padding:15px;border-radius:8px;margin-bottom:20px;'>"
"<p style='margin:0 0 10px 0;font-size:15px;color:#fbbf24;'><strong>📦 10 GB Free Storage</strong></p>"
"<p style='margin:0;font-size:14px;color:#cbd5e1;line-height:1.6;'>D1 works alongside Adafruit IO. Adafruit shows real-time data, D1 archives it forever.</p>"
"</div>"
"<h3 style='color:#f1f5f9;margin-bottom:15px;font-size:16px;'>📋 Setup Steps</h3>"
"<div style='background:#1e293b;padding:15px;border-radius:8px;margin-bottom:20px;'>"
"<ol style='margin:0 0 0 20px;padding:0;color:#cbd5e1;font-size:14px;line-height:2;'>"
"<li><strong>Install Wrangler CLI:</strong><br>"
"<code style='background:#0f172a;padding:3px 8px;border-radius:4px;color:#10b981;font-family:monospace;font-size:12px;'>npm install -g wrangler</code></li>"
"<li><strong>Navigate to cloudflare folder:</strong><br>"
"<code style='background:#0f172a;padding:3px 8px;border-radius:4px;color:#10b981;font-family:monospace;font-size:12px;'>cd MijiaESP32Hub/cloudflare</code></li>"
"<li><strong>Run setup script:</strong><br>"
"<code style='background:#0f172a;padding:3px 8px;border-radius:4px;color:#10b981;font-family:monospace;font-size:12px;'>./setup.sh</code><br>"
"<span style='color:#94a3b8;font-size:13px;'>Script will authenticate, create database, and deploy Worker</span></li>"
"<li><strong>Copy credentials:</strong> Script shows Worker URL and API Token at the end</li>"
"<li><strong>Paste above:</strong> Enter values in Configuration section and enable archive</li>"
"<li><strong>Test:</strong> Click <strong>🧪 Test Connection</strong> to verify</li>"
"</ol>"
"</div>"
"<div style='background:#1e3a4c;border-left:4px solid #3b82f6;padding:15px;border-radius:8px;margin-bottom:20px;'>"
"<p style='margin:0;font-size:14px;color:#bfdbfe;'><strong>💡 Free Tier Limits:</strong></p>"
"<ul style='margin:10px 0 0 20px;color:#93c5fd;line-height:1.8;'>"
"<li><strong>10 GB</strong> storage</li>"
"<li><strong>100,000</strong> writes per day</li>"
"<li><strong>~300 devices</strong> at 5 min intervals</li>"
"<li><strong>No automatic deletion</strong> - data stays forever</li>"
"</ul>"
"</div>"
"<div style='background:#0f172a;border-left:4px solid #10b981;padding:15px;border-radius:8px;margin-bottom:20px;'>"
"<p style='margin:0 0 10px 0;font-size:14px;color:#6ee7b7;'><strong>📊 Tietojen lukeminen</strong></p>"
"<p style='margin:0;font-size:13px;color:#cbd5e1;line-height:1.6;'>Voit kysellä tietoja Wranglerilla:</p>"
"<code style='display:block;background:#1e293b;padding:10px;border-radius:6px;color:#10b981;font-family:monospace;font-size:12px;margin-top:10px;'>wrangler d1 execute mijaesp32hub --command=\"SELECT * FROM device_aabbccddeeff LIMIT 10\"</code>"
"</div>"
"<button onclick='closeD1Help()' style='width:100%;padding:12px;background:#f59e0b;color:white;border:none;border-radius:8px;font-weight:600;cursor:pointer;'>✓ OK</button>"
"</div></div></div>"
"<div id='chartModal' class='modal'><div class='modal-content' style='max-width:900px;'>"
"<div class='modal-header'><h2 id='chartTitle'>📊 Temperature history</h2>"
