...
"html+=' </div>'"
"+' <button class=\"settings-icon\" onclick=\"toggleSettings(\\''+addrClean+'\\')\" >⚙️</button>'"
...
"html+=' <button onclick=\"openChart(\\''+name.replace(/'/g,'\\\\\\'')+'\\',\\''+d.addr+'\\')\" style=\"margin:16px 0 0 0;padding:10px;width:100%;background:#3b82f6;color:white;border:none;border-radius:8px;cursor:pointer;font-weight:600;\">📊 Näytä historia (48h)</button>';}"
"html+='</div>';});"
...
"async function openChart(name,addr){"
"document.getElementById('chartModal').classList.add('show');"
"document.getElementById('chartTitle').textContent='📊 '+name+' - Lämpötilahistoria (48h)';"
...
"const url=`https://io.adafruit.com/api/v2/${cfg.username}/feeds/${feedName}/data?start_time=${new Date(Date.now()-48*3600*1000).toISOString()}`;"
...
"currentChart=new Chart(ctx,{type:'line',data:{datasets:[{label:'Lämpötila',data:temps,borderColor:'#3b82f6',backgroundColor:'rgba(59,130,246,0.1)',tension:0.4,fill:true,pointRadius:2}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{type:'time',time:{unit:'hour',displayFormats:{hour:'HH:mm'}},ticks:{color:'#94a3b8'}},y:{ticks:{color:'#94a3b8',callback:v=>v+'°C'}}}}});"
